...
from flask import request, redirect, url_for, render_template, flash, make_response, abort, Response
...
@login_required
def get_account(username: str) -> str:
...
@login_required
def upload_avatar() -> Union[Response, str]:
    if request.method == 'POST':
...
def register() -> Response:
  ...
  if request.method == "POST":
    if form.validate_on_submit():
      ...
      db.session.commit()
      code = new_user.login[:3] + str(new_user.id) + str(generate_random())
      new_user.code = code
      db.session.commit()
      login_user(new_user)
      return redirect(url_for("main"))
